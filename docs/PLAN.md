...
| Item | Notes |
|---|---|
| ~~"Category" naming collision (client UI = `envelopes`)~~ — **resolved by deletion, not rename** | Investigating the rename scoped below turned up that the admin `categories` catalog (B-06) was never wired to any feature — `claudeService.buildPrompt()` classifies against the user's `envelopes` only, and `transactions` has no column to persist a category label. Rather than rename a dead table, it was dropped entirely (migration `20260820000400-drop-categories.js`; see `docs/DATABASE.md` § Migration Conventions). Only one concept is now called "Category," so the collision is gone and the client rename below is **cancelled, not performed** — the ~25-file scope (`CategoryCard.jsx`→`EnvelopeCard.jsx` etc.) described in the original entry never happened and should not be picked up. The two free wins from that entry (dead `"categories"` locale namespace, stale `categoryLabel.js` comment) were bundled into the removal commit instead. |
